...
#need better error handling for url and whatnot
...
def initialCategorySeriesUpload(**kwargs):
	...
	for x in categoryList:
		...
		#******** not sure if needed...don't over bang the api, one sec per cat
		time.sleep(1)
...
def xmlCategoryParser(url):
	try:
		...
		if err.code == 400:
			...
			print "error 400"
			print "the following url failed to load: " + url
			return None	
		else:
			raise 
	...
	for x in seriesList:
		...
		if len(cursor.fetchall()) <= 0:
			...
			cursor.execute("INSERT INTO fredindicators (FREDID, IndicatorName, ReportingPeriod, \
				...
				x.attributes['units'].value.replace("'","''"), categoryName.replace("'","''"), x.attributes['popularity'].value,
